...
pub fn build(b: *std.Build) void {
    ...
    const Backend = enum { vbisam, sqlite, both, none };
    const backend = b.option(Backend, "backend", "ISAM backend: vbisam|sqlite|both|none") orelse .vbisam;

    const options = b.addOptions();
    options.addOption(Backend, "backend", backend);

    var vbisam_lib: ?*std.Build.Step.Compile = null;
    ...
    if (backend == .sqlite or backend == .both) {
    ...
    if (backend == .vbisam or backend == .both) {
        ...
        vbisam_test_mod.addOptions("build_options", options);
