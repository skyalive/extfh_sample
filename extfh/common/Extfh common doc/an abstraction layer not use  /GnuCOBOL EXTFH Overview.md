...
### Note (this grfh repository)

- 本リポジトリの EXTFH ハンドラは `-fcallfh=grfh` を想定しています（Zig 側: `grfh/src/grfh.zig`）。
- ランタイム/ABI の差で EXTFH が `(opcode_ptr, fcd_ptr)` ではなく `(fcd_ptr)` の1引数で呼ばれるケースがあり得るため、`GRFH_FH` は防御的に両方を受けられる実装にしています。
